...
describe('ArticlesService 유닛 테스트', () => {
  afterEach(() => {
  ...
  describe('deleteArticle', () => {
    ...
    it('작성자가 아닌 사용자가 삭제 시 ForbiddenError를 던져야 한다', async () => {
      const mockArticle = { id: 1, userId: 10 }; // 주인은 10번
      jest.spyOn(articlesRepository, 'getArticle').mockResolvedValue(mockArticle as any);

      await expect(articlesService.deleteArticle(1, 99)) // 99번이 삭제 시도
        .rejects.toThrow(ForbiddenError);
    });

    it('정상적인 삭제 요청 시 repository의 delete 메서드가 호출되어야 한다', async () => {
